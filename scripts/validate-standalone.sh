...
suffix="Standalone"
tree_name="demo_data_sfra_$suffix"

"$node_bin" "$REPO_ROOT/generate.mjs" --suffix "$suffix" --out "$source_out"
PATH=/path-that-does-not-exist "$binary_path" -s "$suffix" --out "$standalone_out"

diff -qr "$source_out/$tree_name" "$standalone_out/$tree_name"
cmp "$source_out/inventory_$suffix.xml" "$standalone_out/inventory_$suffix.xml"
"$unzip_bin" -tqq "$standalone_out/$tree_name.zip"
"$unzip_bin" -tqq "$standalone_out/inventory_$suffix.zip"
...
"$unzip_bin" -qq "$standalone_out/inventory_$suffix.zip" -d "$inventory_extract"
diff -qr "$standalone_out/$tree_name" "$site_extract/$tree_name"
cmp "$standalone_out/inventory_$suffix.xml" "$inventory_extract/inventory_$suffix.xml"
