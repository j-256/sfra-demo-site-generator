...
// Everything here is copied byte-for-byte, never transformed, even when it is XML and even when
// its content happens to contain a substring that also appears as a mapped id. Matches the
// "Untouched files" list in the project's global constraints
function isUntouchedPath(relPosix) {
  const segments = relPosix.split('/');
  if (segments.includes('active-data')) return true;
  if (segments[0] === 'meta') return true;
  if (segments.includes('urls')) return true;
  if (segments[0] === 'geolocations') return true;
  if (relPosix.endsWith('.csv') || relPosix.endsWith('.sample')) return true;
  return false;
}
