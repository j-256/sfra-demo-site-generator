...
export function transformFileContent(xml, rename, { isSiteXml = false, siteName = null, rebrand = false } = {}) {
  ...
  if (rebrand) {
    ...
    // so site-id="RefArch" attributes are already tokenized and never seen here; attribute values
    // are never touched because the regex matches only text between tags. The token lookahead in
    // rebrandProse keeps this idempotent
    out = out.replace(/>([^<]*)</g, (full, text) => {
      if (SOURCE_SITE_NAMES.some((n) => text.includes(n))) {
        return '>' + rebrandProse(text, SOURCE_SITE_NAMES, rename.primaryName, rename.token) + '<';
      }
      return full;
    });
  }
