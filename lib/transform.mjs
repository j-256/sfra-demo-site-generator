...
// function therefore only needs to name the directories whose XML must STILL be copied verbatim
// rather than transformed: meta/*.xml, geolocations/*.xml, and anything nested under a real
// active-data directory. There are two such directories in the vendored data - "active-data" and
// "active-data-lite" - so the match is bounded to either the exact name or a "active-data-"
// prefixed variant (not a bare substring match, to avoid over-matching an unrelated directory
