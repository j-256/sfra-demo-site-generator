...
// A cache-settings file carries all three environment blocks and an import takes all three -
// the instance simply reads whichever block it IS. So enabling caching is per-environment, not
// a profile choice. Production is always on; staging and development are opt-in via --cache.
...
function optionValue(argv, i, flagName) {
  ...
  if (arg.startsWith('--') && eq !== -1) {
    ...
    if (!value) throw usageError(`${flagName} requires a value. Run \`generate.mjs --help\` for usage`);
    return { value, consumed: 1 };
    }
  const value = argv[i + 1];
  if (value === undefined || value.startsWith('-')) {
    throw usageError(`${flagName} requires a value. Run \`generate.mjs --help\` for usage`);
  }
