...
const MAX_SUFFIX = 19; // RefArchGlobal(13) + suffix <= 32 site-id cap
...
// Short letters that TAKE A VALUE, so -cstg expands to -c stg rather than splitting each letter
// -O is uppercase because -o is already --out
const VALUE_SHORTS = 'sOoc';
...
// Expand bundled and glued short options so the parse loop only ever sees one flag per argument:
// -kf becomes -k -f, and -sx becomes -s x when s takes a value. A value that is exactly "-" or
...
export function parseOptions(rawArgv) {
  ...
  const o = {
    help: false, suffix: null, only: null, keepAllocationTimestamps: false, out: 'out',
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (flag === '-s' || flag === '--suffix') {
      const { value, consumed } = optionValue(argv, i, '--suffix');
      o.suffix = value; i += consumed - 1;
  ...
  if (!o.suffix) throw usageError('suffix is required (use --suffix <suffix>)');
  if (!SUFFIX_RE.test(o.suffix)) throw usageError(`invalid suffix "${o.suffix}": use only [A-Za-z0-9_-]`);
  if (o.suffix.length > MAX_SUFFIX) throw usageError(`suffix too long: max ${MAX_SUFFIX} chars`);
