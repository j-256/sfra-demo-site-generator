...
// Attribute names whose VALUES are root IDs we rename. category-id is deliberately excluded.
const ID_ATTRS = ['site-id', 'catalog-id', 'product-id', 'source-id', 'target-id', 'list-id', 'pricebook-id', 'library-id', 'store-id'];
// Element text carrying root IDs (single-valued).
const ID_ELEMENTS = ['parent', 'SiteCatalog', 'SiteCustomerList', 'SiteLibrary', 'SiteInventoryList', 'inventoryListId'];
...
export function harvestIds(srcDir) {
  ...
  for (const file of walk(srcDir)) {
    ...
    const prefRe = /<preference preference-id="(SiteCatalog|SiteCustomerList|SiteLibrary|SiteInventoryList)">([^<]+)<\/preference>/g;
    let pm;
    while ((pm = prefRe.exec(xml))) ids.add(pm[2].trim());
    // SitePriceBooks is a colon-delimited list
    const pbRe = /<preference preference-id="SitePriceBooks">([^<]+)<\/preference>/g;
    let pbm;
