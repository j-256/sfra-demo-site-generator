...
export function verifyTree(dir) {
  ...
  for (const f of allXml(dir)) {
    ...
    // suffixed id. This checker does not attempt to catch that case either - a --only run's
    ...
    // which already collect/rename every entry in ID_ATTRS. Real vendored data confirms this is safe: a
